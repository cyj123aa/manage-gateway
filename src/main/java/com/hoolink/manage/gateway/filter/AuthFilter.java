...
import com.hoolink.manage.gateway.consumer.ManageBaseClient;
...
public class AuthFilter implements GlobalFilter, Ordered {
    ...
    @Resource
    private ManageBaseClient manageBaseClient;
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if(StringUtils.isNotBlank(uri) && !AuthConfig.getPassOperations().contains(uri)){
            ...
            if(StringUtils.isNotBlank(mobileToken)){
                currentUserBO=manageBaseClient.getSessionUser(token,true);
            }else{
                if(StringUtils.isNotBlank(token)) {
                    currentUserBO=manageBaseClient.getSessionUser(token,false);
                }else{
                    currentUserBO=manageBaseClient.getSessionUser("",false);
                }
            }
        }
